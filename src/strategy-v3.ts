...
export function calculateStrategyV2(
  ...
  // ===================== 1. EMA三线趋势判断（五档）
  // EMA120 在数据不足时 fallback 到 EMA60
  const ema20  = calcEMA(closes, 20);
  const ema60  = calcEMA(closes, Math.min(60, dataLen - 1));
  const ema120 = calcEMA(closes, Math.min(120, dataLen - 1));
  ...
  const bollLines = calcBOLL_Lines(closes);
  const indicators = {
    ma5:       ma5.map((v, i)   => i < 4  ? null : Math.round(v * 100) / 100),
    ema20:     ema20.map((v, i) => i < 19 ? null : Math.round(v * 100) / 100),
    ema60:     ema60.map((v, i) => i < 59 ? null : Math.round(v * 100) / 100),
    bollUpper: bollLines.upper,
    bollMid:   bollLines.mid,
    bollLower: bollLines.lower,
  };
...
function calcMA_Signal(
  ...
  const sell: string[] = [];
  const ma5 = calcSMA(closes, 5);
  const startIdx = Math.min(60, Math.floor(closes.length * 0.4));
...
// ===================== BOLL三线数值序列（供前端画线）
function calcBOLL_Lines(closes: number[]): {
  upper: (number | null)[];
  mid:   (number | null)[];
  lower: (number | null)[];
} {
  ...
  for (let i = 0; i < closes.length; i++) {
    ...
    const std = Math.sqrt(slice.reduce((s, v) => s + (v - m) ** 2, 0) / 20);
    upper.push(Math.round((m + 2 * std) * 100) / 100);
    mid.push(Math.round(m * 100) / 100);
    lower.push(Math.round((m - 2 * std) * 100) / 100);
  }
